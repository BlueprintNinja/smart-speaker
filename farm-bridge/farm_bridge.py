...
def run_once():
    ...
    solar_rad  = float(current.get("shortwave_radiation") or 0)
    precip     = float(current.get("precipitation")      or 0)
    soil_moisture = float(current.get("soil_moisture_0_to_7cm") or 0.3)
    soil_temp  = float(current.get("soil_temperature_18cm") or 0)
